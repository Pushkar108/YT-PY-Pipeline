...
# Function to load data from DuckDB database
def load_data(db_file):
    # Connect to DuckDB database
    conn = duckdb.connect(database=db_file)
    # Execute a query to select all data from the video_metrics table
    result = conn.execute("SELECT * FROM video_metrics")
    # Fetch all rows from the result
    rows = result.fetchall()
    # Convert data to DataFrame
    data = pd.DataFrame(rows, columns=['Video ID', 'Title', 'Upload Date', 'Views', 'Likes', 'Dislikes', 'Comments'])
    return data
...
def main():
    ...
    # Upload .db file
    db_file = st.file_uploader("Upload DuckDB file", type=["db"])
